...
export default function Profile({ user, tgUser }) {
  ...
  useEffect(() => {
    ...
    if (user) {
      setForm({ name: user.name || '', calories: user.calories || 2000, goal: user.goal || 'health' })
    }
  }, [user])
  ...
  const menuItems = [
    ...
    { icon: 'ti-target', color: 'green', label: 'Цели и параметры', action: () => haptic('light') },
    { icon: 'ti-stethoscope', color: 'pink', label: 'Консультации', badge: 'Скоро', action: () => haptic('light') },
    { icon: 'ti-robot', color: 'green', label: 'ИИ-ассистент SOFE', badge: 'Beta', action: () => haptic('light') },
    ...
    <div className="screen">
      ...
      <div style={{ background: 'var(--pink)', borderRadius: 'var(--radius)', padding: 20, color: 'white', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontSize: 12, opacity: 0.85, marginTop: 3 }}>
          {goalLabel[user?.goal] || '❤️ Здоровье'} · {form.calories} ккал/день
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        <div className="card" style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 16, fontWeight: 900, color: 'var(--pink)' }}>1 871</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>ккал сегодня</div>
        </div>
        <div className="card" style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 22, marginBottom: 4 }}>💧</div>
          <div style={{ fontSize: 16, fontWeight: 900, color: '#3498db' }}>1.8 л</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>воды сегодня</div>
        </div>
      </div>
        ...
        {menuItems.map((item, i) => (
          <div
            ...
            style={{
              ...
              borderBottom: i < menuItems.length - 1 ? '0.5px solid var(--border)' : 'none',
              cursor: 'pointer'
            }}
          >
            <div style={{
              width: 32, height: 32, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center',
              background: item.color === 'pink' ? 'var(--pink-light)' : item.color === 'green' ? 'var(--green-light)' : '#FFF3E0'
            }}>
